...
class DerivingPlugin(global: Global) extends AnnotationPlugin(global) { self =>
  ...
  def updateClass(triggered: List[Mod.Annot], klass: Defn.Class): Defn.Class = klass

  def updateCompanion(triggered: List[Mod.Annot], klass: Defn.Class, companion: Defn.Object): Defn.Object =
    regenModule(companion,
      if (klass.mods.has[Mod.Sealed]) mkStats(CopTree(klass, companion, _), triggered)
      else if (klass.mods.has[Mod.Case]) mkStats(ProdTree(klass, _), triggered)
      else Nil)

  def updateModule(triggered: List[Mod.Annot], module: Defn.Object): Defn.Object = module
  ...
  def getChildrenOfTpe(tpeName: Type.Name, companion: Defn.Object, owner: Option[Tree]): List[Either[Defn.Class, Defn.Object]] =
    (companion.extract[Stat] ++ owner.map(_ match {
  ...
  object CopTree {
    def apply(name: Type.Name, companion: Defn.Object, owner: Option[Tree], tparams: List[Type.Param], labelled: Boolean): CopTree =
      new CopTree(getChildrenOfTpe(name, companion, owner), name, tparams, labelled)

    def apply(c: Defn.Class, companion: Defn.Object, l: Boolean): GenTree =
      apply(c.name, companion, c.owner, c.tparams, l)

    def apply(t: Defn.Trait, companion: Defn.Object, l: Boolean): GenTree =
      apply(t.name, companion, t.owner, t.tparams, l)
  }
  ...
  def mkStats(mkTree: Boolean => GenTree, mods: List[Mod.Annot]): List[Stat] = {
