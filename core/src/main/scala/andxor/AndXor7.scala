...
trait AndXor7[A1, A2, A3, A4, A5, A6, A7] extends AndXor {
  ...
  object Prod { def apply[F[_]](p: (F[A1], (F[A2], (F[A3], (F[A4], (F[A5], (F[A6], F[A7]))))))): Prod[F] = Prod7(p) }

  type Cop[F[_]] = Cop7[F, A1, A2, A3, A4, A5, A6, A7]
  object Cop { def apply[F[_]](c: (F[A1] \/ (F[A2] \/ (F[A3] \/ (F[A4] \/ (F[A5] \/ (F[A6] \/ F[A7]))))))): Cop[F] = Cop7(c) }
  ...
  def transformP[F[_], G[_]](nt: (F ~> G)): AndXor7[A1, A2, A3, A4, A5, A6, A7]#Prod[F] => AndXor7[A1, A2, A3, A4, A5, A6, A7]#Prod[G] =
    (p: AndXor7[A1, A2, A3, A4, A5, A6, A7]#Prod[F]) => {
      val pr = p.run
      Prod[G]((nt(pr.t1), (nt(pr.t2), (nt(pr.t3), (nt(pr.t4), (nt(pr.t5), (nt(pr.t6), nt(pr.t7))))))))
    }

  def transformC[F[_], G[_]](nt: (F ~> G)): AndXor7[A1, A2, A3, A4, A5, A6, A7]#Cop[F] => AndXor7[A1, A2, A3, A4, A5, A6, A7]#Cop[G] =
    (p: AndXor7[A1, A2, A3, A4, A5, A6, A7]#Cop[F]) => Cop[G](p.run.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), nt(_))))))))

  def subst1[B]: AndXor7[B, A2, A3, A4, A5, A6, A7] = AndXor7[B, A2, A3, A4, A5, A6, A7]

  def subst2[B]: AndXor7[A1, B, A3, A4, A5, A6, A7] = AndXor7[A1, B, A3, A4, A5, A6, A7]

  def subst3[B]: AndXor7[A1, A2, B, A4, A5, A6, A7] = AndXor7[A1, A2, B, A4, A5, A6, A7]

  def subst4[B]: AndXor7[A1, A2, A3, B, A5, A6, A7] = AndXor7[A1, A2, A3, B, A5, A6, A7]

  def subst5[B]: AndXor7[A1, A2, A3, A4, B, A6, A7] = AndXor7[A1, A2, A3, A4, B, A6, A7]

  def subst6[B]: AndXor7[A1, A2, A3, A4, A5, B, A7] = AndXor7[A1, A2, A3, A4, A5, B, A7]

  def subst7[B]: AndXor7[A1, A2, A3, A4, A5, A6, B] = AndXor7[A1, A2, A3, A4, A5, A6, B]
  ...
  def foldMap[F[_], C](p: Prod[F])(map: Cop[F] => C)(implicit M: Monoid[C]): C = {
    val pr = p.run
    M.append(map(inj(pr.t1)), M.append(map(inj(pr.t2)), M.append(map(inj(pr.t3)), M.append(map(inj(pr.t4)), M.append(map(inj(pr.t5)), M.append(map(inj(pr.t6)), map(inj(pr.t7))))))))
  }
  ...
  def foldMapId[F[_], C](p: Prod[F])(map: Cop[Id] => C)(
    ...
    def uncons(p: Prod[F]): (List[Cop[Id]], Prod[F]) = {
      ...
      (List(ht1._1.map(inj(_: Id[A1])), ht2._1.map(inj(_: Id[A2])), ht3._1.map(inj(_: Id[A3])), ht4._1.map(inj(_: Id[A4])), ht5._1.map(inj(_: Id[A5])), ht6._1.map(inj(_: Id[A6])), ht7._1.map(inj(_: Id[A7]))).flatten,
        Prod[F]((ht1._2, (ht2._2, (ht3._2, (ht4._2, (ht5._2, (ht6._2, ht7._2))))))))
    }
        ...
        case false => q.isEmpty match {
          ...
          case false => q.dequeue.run match {
            case -\/(x) => {
              val pr = prod.run
              val (h, t) = U(pr.t1)
              go(Prod((t, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, pr.t7))))))),
                q ++= h.map(inj(_: Id[A1])), M.append(out, map(inj(x))))
          }
          case \/-(-\/(x)) => {
              val pr = prod.run
              val (h, t) = U(pr.t2)
              go(Prod((pr.t1, (t, (pr.t3, (pr.t4, (pr.t5, (pr.t6, pr.t7))))))),
                q ++= h.map(inj(_: Id[A2])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(-\/(x))) => {
              val pr = prod.run
              val (h, t) = U(pr.t3)
              go(Prod((pr.t1, (pr.t2, (t, (pr.t4, (pr.t5, (pr.t6, pr.t7))))))),
                q ++= h.map(inj(_: Id[A3])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(-\/(x)))) => {
              val pr = prod.run
              val (h, t) = U(pr.t4)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (t, (pr.t5, (pr.t6, pr.t7))))))),
                q ++= h.map(inj(_: Id[A4])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(-\/(x))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t5)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (t, (pr.t6, pr.t7))))))),
                q ++= h.map(inj(_: Id[A5])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(-\/(x)))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t6)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (t, pr.t7))))))),
                q ++= h.map(inj(_: Id[A6])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(\/-(x)))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t7)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, t))))))),
