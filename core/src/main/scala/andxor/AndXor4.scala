...
trait AndXorK4[F[_], A1, A2, A3, A4] extends AndXor {
  ...
  def transformP[G[_]](nt: (F ~> G)): AndXorK4[F, A1, A2, A3, A4]#Prod => AndXorK4[G, A1, A2, A3, A4]#Prod =
    (p: AndXorK4[F, A1, A2, A3, A4]#Prod) => Prod4[G, A1, A2, A3, A4]((nt(p.t1), nt(p.t2), nt(p.t3), nt(p.t4)))
  ...
  // format: off
  def sequenceP(p: Prod)(implicit A: Apply[F]): F[Prod4[Id, A1, A2, A3, A4]] = {
    A.map(
    A.ap(p.t4)(
    A.ap(p.t3)(
    A.ap(p.t2)(
  ...
  def foldMap[G[_], C](p: AndXor[G]#Prod)(map: AndXor[Id]#Cop => C)(
      implicit O: Ordering[Cop4[Id, A1, A2, A3, A4]], M: Monoid[C], PE: PlusEmpty[G], U: Uncons[G]): C = {
