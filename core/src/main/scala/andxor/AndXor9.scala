...
trait AndXor9[A1 <: AndXor, A2 <: AndXor, A3 <: AndXor, A4 <: AndXor, A5 <: AndXor, A6 <: AndXor, A7 <: AndXor, A8 <: AndXor, A9 <: AndXor] extends AndXor {
  ...
  object Cop {
    def apply[F[_]](c: (A1#Cop[F] \/ (A2#Cop[F] \/ (A3#Cop[F] \/ (A4#Cop[F] \/ (A5#Cop[F] \/ (A6#Cop[F] \/ (A7#Cop[F] \/ (A8#Cop[F] \/ A9#Cop[F]))))))))): Cop[F] =
      Cop9[F, A1, A2, A3, A4, A5, A6, A7, A8, A9](c)
  }
  ...
  def foldMap[F[_], C](p: Prod[F])(map: Cop[Id] => C)(
    ...
    PE: PlusEmpty[F],
    U: Uncons[F],
    U0: Uncons0[A1#Prod, A1#Cop], U1: Uncons0[A2#Prod, A2#Cop], U2: Uncons0[A3#Prod, A3#Cop], U3: Uncons0[A4#Prod, A4#Cop], U4: Uncons0[A5#Prod, A5#Cop], U5: Uncons0[A6#Prod, A6#Cop], U6: Uncons0[A7#Prod, A7#Cop], U7: Uncons0[A8#Prod, A8#Cop], U8: Uncons0[A9#Prod, A9#Cop]
  ): C = {
    ...
    def uncons(p: Prod[F]): (List[Cop[Id]], Prod[F]) = {
      ...
      val (h8, t8) = U7(p.t8)
      val (h9, t9) = U8(p.t9)
      (List(h1.map(inj(_: A1#Cop[Id])), h2.map(inj(_: A2#Cop[Id])), h3.map(inj(_: A3#Cop[Id])), h4.map(inj(_: A4#Cop[Id])), h5.map(inj(_: A5#Cop[Id])), h6.map(inj(_: A6#Cop[Id])), h7.map(inj(_: A7#Cop[Id])), h8.map(inj(_: A8#Cop[Id])), h9.map(inj(_: A9#Cop[Id]))).flatten,
        Prod[F]((t1, t2, t3, t4, t5, t6, t7, t8, t9)))
    }
      ...
      (prod.run.==((PE.empty[A1], PE.empty[A2], PE.empty[A3], PE.empty[A4], PE.empty[A5], PE.empty[A6], PE.empty[A7], PE.empty[A8], PE.empty[A9]))) match {
        ...
        case false => q.isEmpty match {
          ...
          case false => q.dequeue.run match {
            case dj @ -\/(_) =>
              val (h, t) = U0(prod.t1)
              go(Prod[F]((t, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A1#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(-\/(_)) =>
              val (h, t) = U1(prod.t2)
              go(Prod[F]((prod.t1, t, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A2#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(-\/(_))) =>
              val (h, t) = U2(prod.t3)
              go(Prod[F]((prod.t1, prod.t2, t, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A3#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(-\/(_)))) =>
              val (h, t) = U3(prod.t4)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, t, prod.t5, prod.t6, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A4#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(-\/(_))))) =>
              val (h, t) = U4(prod.t5)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, t, prod.t6, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A5#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(-\/(_)))))) =>
              val (h, t) = U5(prod.t6)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, t, prod.t7, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A6#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(\/-(-\/(_))))))) =>
              val (h, t) = U6(prod.t7)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, t, prod.t8, prod.t9)),
                q ++= h.map(inj(_: A7#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(\/-(\/-(-\/(_)))))))) =>
              val (h, t) = U7(prod.t8)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, t, prod.t9)),
                q ++= h.map(inj(_: A8#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(\/-(\/-(\/-(_)))))))) =>
              val (h, t) = U8(prod.t9)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8, t)),
