...
trait AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10] extends AndXor {
  ...
  object Prod { def apply[F[_]](p: (F[A1], (F[A2], (F[A3], (F[A4], (F[A5], (F[A6], (F[A7], (F[A8], (F[A9], F[A10])))))))))): Prod[F] = Prod10(p) }

  type Cop[F[_]] = Cop10[F, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]
  object Cop { def apply[F[_]](c: (F[A1] \/ (F[A2] \/ (F[A3] \/ (F[A4] \/ (F[A5] \/ (F[A6] \/ (F[A7] \/ (F[A8] \/ (F[A9] \/ F[A10])))))))))): Cop[F] = Cop10(c) }
  ...
  def transformP[F[_], G[_]](nt: (F ~> G)): AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Prod[F] => AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Prod[G] =
    (p: AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Prod[F]) => {
      val pr = p.run
      Prod[G]((nt(pr.t1), (nt(pr.t2), (nt(pr.t3), (nt(pr.t4), (nt(pr.t5), (nt(pr.t6), (nt(pr.t7), (nt(pr.t8), (nt(pr.t9), nt(pr.t10)))))))))))
    }
  ...
  def transformC[F[_], G[_]](nt: (F ~> G)): AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Cop[F] => AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Cop[G] =
    (p: AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, A10]#Cop[F]) =>
      Cop[G](p.run.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), nt(_)))))))))))

  def subst1[B]: AndXor10[B, A2, A3, A4, A5, A6, A7, A8, A9, A10] = AndXor10[B, A2, A3, A4, A5, A6, A7, A8, A9, A10]

  def subst2[B]: AndXor10[A1, B, A3, A4, A5, A6, A7, A8, A9, A10] = AndXor10[A1, B, A3, A4, A5, A6, A7, A8, A9, A10]

  def subst3[B]: AndXor10[A1, A2, B, A4, A5, A6, A7, A8, A9, A10] = AndXor10[A1, A2, B, A4, A5, A6, A7, A8, A9, A10]

  def subst4[B]: AndXor10[A1, A2, A3, B, A5, A6, A7, A8, A9, A10] = AndXor10[A1, A2, A3, B, A5, A6, A7, A8, A9, A10]

  def subst5[B]: AndXor10[A1, A2, A3, A4, B, A6, A7, A8, A9, A10] = AndXor10[A1, A2, A3, A4, B, A6, A7, A8, A9, A10]

  def subst6[B]: AndXor10[A1, A2, A3, A4, A5, B, A7, A8, A9, A10] = AndXor10[A1, A2, A3, A4, A5, B, A7, A8, A9, A10]

  def subst7[B]: AndXor10[A1, A2, A3, A4, A5, A6, B, A8, A9, A10] = AndXor10[A1, A2, A3, A4, A5, A6, B, A8, A9, A10]

  def subst8[B]: AndXor10[A1, A2, A3, A4, A5, A6, A7, B, A9, A10] = AndXor10[A1, A2, A3, A4, A5, A6, A7, B, A9, A10]

  def subst9[B]: AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, B, A10] = AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, B, A10]

  def subst10[B]: AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, B] = AndXor10[A1, A2, A3, A4, A5, A6, A7, A8, A9, B]
  ...
  def foldMap[F[_], C](p: Prod[F])(map: Cop[F] => C)(implicit M: Monoid[C]): C = {
    val pr = p.run
    M.append(map(inj(pr.t1)), M.append(map(inj(pr.t2)), M.append(map(inj(pr.t3)), M.append(map(inj(pr.t4)), M.append(map(inj(pr.t5)), M.append(map(inj(pr.t6)), M.append(map(inj(pr.t7)), M.append(map(inj(pr.t8)), M.append(map(inj(pr.t9)), map(inj(pr.t10)))))))))))
  }
  ...
  def foldMapId[F[_], C](p: Prod[F])(map: Cop[Id] => C)(
    ...
    def uncons(p: Prod[F]): (List[Cop[Id]], Prod[F]) = {
      ...
      (List(ht1._1.map(inj(_: Id[A1])), ht2._1.map(inj(_: Id[A2])), ht3._1.map(inj(_: Id[A3])), ht4._1.map(inj(_: Id[A4])), ht5._1.map(inj(_: Id[A5])), ht6._1.map(inj(_: Id[A6])), ht7._1.map(inj(_: Id[A7])), ht8._1.map(inj(_: Id[A8])), ht9._1.map(inj(_: Id[A9])), ht10._1.map(inj(_: Id[A10]))).flatten,
        Prod[F]((ht1._2, (ht2._2, (ht3._2, (ht4._2, (ht5._2, (ht6._2, (ht7._2, (ht8._2, (ht9._2, ht10._2)))))))))))
    }
        ...
        case false => q.isEmpty match {
          ...
          case false => q.dequeue.run match {
            case -\/(x) => {
              val pr = prod.run
              val (h, t) = U(pr.t1)
              go(Prod((t, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A1])), M.append(out, map(inj(x))))
          }
          case \/-(-\/(x)) => {
              val pr = prod.run
              val (h, t) = U(pr.t2)
              go(Prod((pr.t1, (t, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A2])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(-\/(x))) => {
              val pr = prod.run
              val (h, t) = U(pr.t3)
              go(Prod((pr.t1, (pr.t2, (t, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A3])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(-\/(x)))) => {
              val pr = prod.run
              val (h, t) = U(pr.t4)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (t, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A4])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(-\/(x))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t5)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (t, (pr.t6, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A5])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(-\/(x)))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t6)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (t, (pr.t7, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A6])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(\/-(-\/(x))))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t7)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (t, (pr.t8, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A7])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(\/-(\/-(-\/(x)))))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t8)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (t, (pr.t9, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A8])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(\/-(\/-(\/-(-\/(x))))))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t9)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (t, pr.t10)))))))))),
                q ++= h.map(inj(_: Id[A9])), M.append(out, map(inj(x))))
          }
          case \/-(\/-(\/-(\/-(\/-(\/-(\/-(\/-(\/-(x))))))))) => {
              val pr = prod.run
              val (h, t) = U(pr.t10)
              go(Prod((pr.t1, (pr.t2, (pr.t3, (pr.t4, (pr.t5, (pr.t6, (pr.t7, (pr.t8, (pr.t9, t)))))))))),
