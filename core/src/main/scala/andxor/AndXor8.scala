...
trait AndXor8[A1 <: AndXor, A2 <: AndXor, A3 <: AndXor, A4 <: AndXor, A5 <: AndXor, A6 <: AndXor, A7 <: AndXor, A8 <: AndXor] extends AndXor {
  ...
  def mkChoose[TC[_], F[_], B](
      f: B => Cop[F]
  )(implicit d: Decidable[TC], a0: TC[A1#Cop[F]], a1: TC[A2#Cop[F]], a2: TC[A3#Cop[F]], a3: TC[A4#Cop[F]], a4: TC[A5#Cop[F]], a5: TC[A6#Cop[F]], a6: TC[A7#Cop[F]], a7: TC[A8#Cop[F]]): TC[B] =
    Combine.choose8(a0, a1, a2, a3, a4, a5, a6, a7)(f(_).run)

  def mkAlt[TC[_], F[_], B](
      f: Cop[F] => B
  )(implicit a: Alt[TC], a0: TC[A1#Cop[F]], a1: TC[A2#Cop[F]], a2: TC[A3#Cop[F]], a3: TC[A4#Cop[F]], a4: TC[A5#Cop[F]], a5: TC[A6#Cop[F]], a6: TC[A7#Cop[F]], a7: TC[A8#Cop[F]]): TC[B] =
    Combine.altly8(a0, a1, a2, a3, a4, a5, a6, a7)(x => f(Cop(x)))

  def mkDivide[TC[_], F[_], B](
      f: B => Prod[F]
  )(implicit d: Divide[TC], a0: TC[A1#Prod[F]], a1: TC[A2#Prod[F]], a2: TC[A3#Prod[F]], a3: TC[A4#Prod[F]], a4: TC[A5#Prod[F]], a5: TC[A6#Prod[F]], a6: TC[A7#Prod[F]], a7: TC[A8#Prod[F]]): TC[B] =
    Combine.divide8(a0, a1, a2, a3, a4, a5, a6, a7)(f(_).run)

  def mkApply[TC[_], F[_], B](
      f: Prod[F] => B
  )(implicit a: Apply[TC], a0: TC[A1#Prod[F]], a1: TC[A2#Prod[F]], a2: TC[A3#Prod[F]], a3: TC[A4#Prod[F]], a4: TC[A5#Prod[F]], a5: TC[A6#Prod[F]], a6: TC[A7#Prod[F]], a7: TC[A8#Prod[F]]): TC[B] =
  ...
  def alt[TC[_], F[_]](implicit a: Alt[TC], a0: TC[A1#Cop[F]], a1: TC[A2#Cop[F]], a2: TC[A3#Cop[F]], a3: TC[A4#Cop[F]], a4: TC[A5#Cop[F]], a5: TC[A6#Cop[F]], a6: TC[A7#Cop[F]], a7: TC[A8#Cop[F]])
  ...
  def foldMap[F[_], C](p: Prod[F])(map: Cop[Id] => C)(
    ...
    PE: PlusEmpty[F],
    U: Uncons[F],
    U0: Uncons0[A1#Prod, A1#Cop], U1: Uncons0[A2#Prod, A2#Cop], U2: Uncons0[A3#Prod, A3#Cop], U3: Uncons0[A4#Prod, A4#Cop], U4: Uncons0[A5#Prod, A5#Cop], U5: Uncons0[A6#Prod, A6#Cop], U6: Uncons0[A7#Prod, A7#Cop], U7: Uncons0[A8#Prod, A8#Cop]
  ): C = {
    ...
    def uncons(p: Prod[F]): (List[Cop[Id]], Prod[F]) = {
      ...
      val (h7, t7) = U6(p.t7)
      val (h8, t8) = U7(p.t8)
      (List(h1.map(inj(_: A1#Cop[Id])), h2.map(inj(_: A2#Cop[Id])), h3.map(inj(_: A3#Cop[Id])), h4.map(inj(_: A4#Cop[Id])), h5.map(inj(_: A5#Cop[Id])), h6.map(inj(_: A6#Cop[Id])), h7.map(inj(_: A7#Cop[Id])), h8.map(inj(_: A8#Cop[Id]))).flatten,
        Prod[F]((t1, t2, t3, t4, t5, t6, t7, t8)))
    }
      ...
      (prod.run.==((PE.empty[A1], PE.empty[A2], PE.empty[A3], PE.empty[A4], PE.empty[A5], PE.empty[A6], PE.empty[A7], PE.empty[A8]))) match {
        ...
        case false => q.isEmpty match {
          ...
          case false => q.dequeue.run match {
            case dj @ -\/(_) =>
              val (h, t) = U0(prod.t1)
              go(Prod[F]((t, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A1#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(-\/(_)) =>
              val (h, t) = U1(prod.t2)
              go(Prod[F]((prod.t1, t, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A2#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(-\/(_))) =>
              val (h, t) = U2(prod.t3)
              go(Prod[F]((prod.t1, prod.t2, t, prod.t4, prod.t5, prod.t6, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A3#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(-\/(_)))) =>
              val (h, t) = U3(prod.t4)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, t, prod.t5, prod.t6, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A4#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(-\/(_))))) =>
              val (h, t) = U4(prod.t5)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, t, prod.t6, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A5#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(-\/(_)))))) =>
              val (h, t) = U5(prod.t6)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, t, prod.t7, prod.t8)),
                q ++= h.map(inj(_: A6#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(\/-(-\/(_))))))) =>
              val (h, t) = U6(prod.t7)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, t, prod.t8)),
                q ++= h.map(inj(_: A7#Cop[Id])), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(\/-(\/-(\/-(_))))))) =>
              val (h, t) = U7(prod.t8)
              go(Prod[F]((prod.t1, prod.t2, prod.t3, prod.t4, prod.t5, prod.t6, prod.t7, t)),
