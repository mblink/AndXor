...
import andxor.types.{Cop5, Prod5, TCDeps5}
...
trait AndXor5[A1, A2, A3, A4, A5] extends AndXor {
  ...
  object Prod { def apply[F[_]](p: (F[A1], F[A2], F[A3], F[A4], F[A5])): Prod[F] = Prod5[F, A1, A2, A3, A4, A5](p) }

  type Cop[F[_]] = Cop5[F, A1, A2, A3, A4, A5]
  object Cop { def apply[F[_]](c: (F[A1] \/ (F[A2] \/ (F[A3] \/ (F[A4] \/ F[A5]))))): Cop[F] = Cop5[F, A1, A2, A3, A4, A5](c) }

  type TCDeps[TC[_], F[_]] = TCDeps5[TC, F, A1, A2, A3, A4, A5]

  def mkChoose[TC[_], F[_], B](f: B => Cop[F])(implicit d: Decidable[TC], tcs: TCDeps[TC, F]): TC[B] =
    Combine.choose5(tcs.a0, tcs.a1, tcs.a2, tcs.a3, tcs.a4)(f(_).run)

  def mkAlt[TC[_], F[_], B](f: Cop[F] => B)(implicit a: Alt[TC], tcs: TCDeps[TC, F]): TC[B] =
    Combine.altly5(tcs.a0, tcs.a1, tcs.a2, tcs.a3, tcs.a4)(x => f(Cop(x)))

  def mkDivide[TC[_], F[_], B](f: B => Prod[F])(implicit d: Divide[TC], tcs: TCDeps[TC, F]): TC[B] =
    Combine.divide5(tcs.a0, tcs.a1, tcs.a2, tcs.a3, tcs.a4)(f(_).run)

  def mkApply[TC[_], F[_], B](f: Prod[F] => B)(implicit a: Apply[TC], tcs: TCDeps[TC, F]): TC[B] =
    Combine.apply5(tcs.a0, tcs.a1, tcs.a2, tcs.a3, tcs.a4) {
      case (i0, i1, i2, i3, i4) =>
  ...
  def transformP[F[_], G[_]](nt: (F ~> G)): Prod[F] => Prod[G] =
    (p: Prod[F]) => Prod[G]((nt(p.t1), nt(p.t2), nt(p.t3), nt(p.t4), nt(p.t5)))

  def transformC[F[_], G[_]](nt: (F ~> G)): Cop[F] => Cop[G] =
    (c: Cop[F]) => Cop[G](c.run.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), _.bimap(nt(_), nt(_))))))
  ...
  def foldMap[G[_], C](p: Prod[G])(map: Cop[Id] => C)(implicit O: Ordering[Cop[Id]], M: Monoid[C], PE: PlusEmpty[G], U: Uncons[G]): C = {
    ...
    def uncons(p: Prod[G]): (List[Cop[Id]], Prod[G]) = {
      ...
      val ht4 = U(p.t4)
      val ht5 = U(p.t5)
      (List(ht1._1.map(injId(_: A1)), ht2._1.map(injId(_: A2)), ht3._1.map(injId(_: A3)), ht4._1.map(injId(_: A4)), ht5._1.map(injId(_: A5))).flatten,
        Prod[G]((ht1._2, ht2._2, ht3._2, ht4._2, ht5._2)))
    }
      ...
      (prod.run.==((PE.empty[A1], PE.empty[A2], PE.empty[A3], PE.empty[A4], PE.empty[A5]))) match {
        ...
        case false => q.isEmpty match {
          ...
          case false => q.dequeue.run match {
            case dj @ -\/(_) =>
              val (h, t) = U(prod.t1)
              go(Prod[G]((t, prod.t2, prod.t3, prod.t4, prod.t5)),
                q ++= h.map(injId(_: A1)), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(-\/(_)) =>
              val (h, t) = U(prod.t2)
              go(Prod[G]((prod.t1, t, prod.t3, prod.t4, prod.t5)),
                q ++= h.map(injId(_: A2)), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(-\/(_))) =>
              val (h, t) = U(prod.t3)
              go(Prod[G]((prod.t1, prod.t2, t, prod.t4, prod.t5)),
                q ++= h.map(injId(_: A3)), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(-\/(_)))) =>
              val (h, t) = U(prod.t4)
              go(Prod[G]((prod.t1, prod.t2, prod.t3, t, prod.t5)),
                q ++= h.map(injId(_: A4)), M.append(out, map(Cop[Id](dj))))
            case dj @ \/-(\/-(\/-(\/-(_)))) =>
              val (h, t) = U(prod.t5)
              go(Prod[G]((prod.t1, prod.t2, prod.t3, prod.t4, t)),
