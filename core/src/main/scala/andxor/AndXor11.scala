...
trait AndXorK11[F[_], A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11] extends AndXor {
  ...
  // format: off
  def sequenceP(prod: Prod)(implicit A: Apply[F]): F[AndXorK11[Id, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11]#Prod] = {
  ...
  def sequenceC(cop: Cop)(implicit FF: Functor[F]): F[AndXorK11[Id, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11]#Cop] =
    cop match {
      case -\/(x) => FF.map(x)(y => -\/(y))
      case \/-(-\/(x)) => FF.map(x)(y => \/-(-\/(y)))
      case \/-(\/-(-\/(x))) => FF.map(x)(y => \/-(\/-(-\/(y))))
